...
%
% 输出显示：
%   泰勒级数展开计算得到的三角函数值，与MATLAB内置三角函数进行比较以及相对误差

if n==1
    inputNum=rad2deg(x);
    [y_sin,mat_sin,error_sin]=my_sin(x);
    disp(['my_sin(', num2str(inputNum), '°) = ', num2str(y_sin),', matlab_sin(', num2str(inputNum), '°) = ', num2str(mat_sin),', 与matlab的sin库函数之间的相对误差 = ', num2str(error_sin)]);
    [y_cos,mat_cos,error_cos]=my_cos(x);
    disp(['my_cos(', num2str(inputNum), '°) = ', num2str(y_cos),', matlab_cos(', num2str(inputNum), '°) = ', num2str(mat_cos),', 与matlab的cos库函数之间的相对误差 = ', num2str(error_cos)]);
else
    [y_arctan,mat_arctan,error_arctan]=my_arctan(x);
    disp(['my_arctan(', num2str(x), ') = ', num2str(y_arctan),'°, matlab_arctan(', num2str(x), ') = ', num2str(mat_arctan),'°, 与matlab的arctan库函数之间的相对误差 = ', num2str(error_arctan),'°']);
    [y_arcsin,mat_arcsin,error_arcsin]=my_arcsin(x);
    disp(['my_arcsin(', num2str(x), ') = ', num2str(y_arcsin),'°, matlab_arcsin(', num2str(x), ') = ', num2str(mat_arcsin),'°, 与matlab的arcsin库函数之间的相对误差 = ', num2str(error_arcsin),'°']);
end
end
...
if x<=1 && x>=-1
    ...
    disp('my_arctan函数精度有限，输入在0附近精度较高');
    result_arctan=NaN;
    mat_arctan=atan(x);
end
